...
class LQZCBusinessTools:
    ...
    async def run_by_route(
        self,
        *,
        route_label: str,
        message: str,
    ) -> list[dict[str, Any]]:
        """Choose and execute business tools for the routed domain.

        这一层非常关键，因为它决定了：
        - 哪些实时业务查询需要被调用
        - 如何根据消息内容抽取订单号、型号等结构化参数
        """
        logger.info(
            "tool.dispatch.start route=%s",
            route_label,
        )
        # 先得到一个“待执行工具队列”。
        hint_queue = self._build_hint_queue(message=message)
        ...
        for hint in hint_queue:
            ...
            logger.info(
                "tool.dispatch.item route=%s tool=%s status=%s",
                route_label,
                execution.tool_name,
                execution.status,
            )
        ...
        logger.info(
            "tool.dispatch.end route=%s tool_count=%s tools=%s",
            route_label,
            len(results),
    ...
    def _build_hint_queue(self, *, message: str) -> list[str]:
        """生成工具调用队列。

        设计思路：
        - 从用户消息里抽型号、订单号、关键词做启发式判断。
