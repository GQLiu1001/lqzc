-- ====================================================================================
--  LQZC 瓷砖商城管理系统 - 完整数据库脚本
--  包含：库存管理、订单系统、C端客户、优惠券、积分、会员等级等所有功能
-- ====================================================================================
...
(
    ...
    `version` TINYINT DEFAULT 0 COMMENT '版本号（乐观锁）',
...
(
    ...
    `customer_id` BIGINT DEFAULT NULL COMMENT 'C端客户ID',
    `customer_phone` VARCHAR(20) DEFAULT '' COMMENT '客户手机号',
    `order_source` TINYINT DEFAULT 1 COMMENT '订单来源(1=前台商城 2=管理后台 3=司机端)',
    `total_price` DECIMAL(10,2) NOT NULL COMMENT '订单总金额(原始金额)',
    `payable_amount` DECIMAL(10,2) NOT NULL DEFAULT 0.00 COMMENT '应付金额(扣除优惠后)',
    `discount_amount` DECIMAL(10,2) NOT NULL DEFAULT 0.00 COMMENT '优惠合计(积分/券)',
    `dispatch_status` INT DEFAULT 0 COMMENT '订单派送状态：0=待派送 1=待接单 2=派送中 3=已完成',
    `order_status` TINYINT NOT NULL DEFAULT 0 COMMENT '订单状态：0=待支付 1=待发货 2=配送中 3=待确认(司机送达) 4=已完成 5=已取消',
    `pay_status` TINYINT NOT NULL DEFAULT 0 COMMENT '支付状态：0=未支付 1=已支付 2=部分退款 3=已退款',
    `pay_channel` TINYINT DEFAULT NULL COMMENT '支付渠道：1=微信 2=支付宝 3=银行卡 4=线下',
    `pay_time` DATETIME DEFAULT NULL COMMENT '支付时间',
    `delivery_fee` DECIMAL(10,2) DEFAULT 0.00 COMMENT '配送费用',
    `driver_id` BIGINT COMMENT '司机ID',
    `delivery_address` VARCHAR(100) DEFAULT NULL COMMENT '派送地址',
    `address_id` BIGINT DEFAULT NULL COMMENT '收货地址ID快照',
    `goods_weight` DECIMAL(10,2) COMMENT '货物重量(吨)',
    `coupon_id` BIGINT DEFAULT NULL COMMENT '使用的优惠券ID',
    `points_used` INT DEFAULT 0 COMMENT '抵扣积分',
...
(
    ...
    `discount_rate` DECIMAL(4,2) DEFAULT 0.00 COMMENT '折扣（小数形式：0.90代表9折）',
...
VALUES (1, 'A8001', '佛山陶瓷一厂', '800x800mm', 1, 2, 1, 349, 4,
       ...
       (15, 'B6009', '广东宏大陶瓷', '600x600mm', 2, 1, 1, 750, 6, 'https://example.com/img/b6002.jpg', 18.00, '哑光防滑墙砖'),
       (16, 'A80012', '佛山陶瓷一厂', '800x800mm', 1, 2, 2, 1000, 4, 'https://example.com/img/a8001.jpg', 25.50, '二号仓备货');
...
VALUES (1, 1, 100, 2550.00),
       (1, 2, 50, 900.00);

INSERT INTO `order_detail` (`order_id`, `item_id`, `amount`, `subtotal_price`)
VALUES (2, 4, 20, 1760.00);
